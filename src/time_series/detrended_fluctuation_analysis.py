...
class DFA(Nonstationary):
    ...
    def mean_fluctuation(self):
        ...
        '''
        fa_2 = np.concatenate((self.squared_fluctuation(self.spl_data, self.time_index_split),self.squared_fluctuation(self.spl_data_reverse, self.time_index_split_reverse)))
        mean = np.mean(fa_2)
        return np.sqrt(mean)
